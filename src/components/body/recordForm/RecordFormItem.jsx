...
export default function RecordFormItem ({labelText, inputName, inputPlaceHolder="", inputType="text", inputValue, onChange}) {
  return(
    <div className="flex flex-col p-2 w-11/12 sm:flex-row">
      <label htmlFor={inputName} className="text-mm p-1 text-left mr-2 ">{labelText}</label>
      <input name={inputName} type={inputType} className="text-mm p-1 w-full rounded placeholder-gray-300" placeholder={inputPlaceHolder} value={inputValue} onChange={onChange}/>
    </div>
  )
}
